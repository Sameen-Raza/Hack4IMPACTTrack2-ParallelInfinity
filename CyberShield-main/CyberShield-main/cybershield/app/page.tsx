...
const handleGetScore = async (e: React.FormEvent) => {
        ...
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.8rem', flexWrap: 'wrap', gap: '0.8rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.7rem' }}>
              {history.length > 0 && <button onClick={clearHistory} style={{ padding: '0.55rem 1rem', background: 'rgba(244,67,54,0.1)', border: '1px solid rgba(244,67,54,0.3)', borderRadius: '8px', color: '#f44336', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '600' }}>Clear</button>}
          ...
          {history.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '0.8rem', marginBottom: '1.5rem' }}>
              {[{ label: 'Total', value: history.length, color: '#07d2f8' }, { label: 'Avg Score', value: avg, color: avg >= 70 ? '#4caf50' : avg >= 40 ? '#ff9800' : '#f44336' }, { label: 'Safe', value: safe, color: '#4caf50' }, { label: 'High Risk', value: risky, color: '#f44336' }].map((s, i) => (
  ...
  // RESULTS
  if (result) {
    const vc = result.overall_score >= 70 ? { color: '#4caf50', label: 'Low Risk', icon: '🛡️' } : result.overall_score >= 40 ? { color: '#ff9800', label: 'Medium Risk', icon: '⚠️' } : { color: '#f44336', label: 'High Risk', icon: '🚨' };
    return (
      <div style={{ minHeight: 'calc(100vh - 120px)', padding: '2rem' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
            ...
            <div style={{ background: 'rgba(7,210,248,0.03)', border: '1px solid rgba(7,210,248,0.2)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              ...
              {hibpResult && (
                <div style={{ marginTop: '0.7rem', padding: '3px 10px', borderRadius: '999px', fontSize: '0.68rem', fontWeight: '600', background: hibpResult.breached ? 'rgba(244,67,54,0.15)' : 'rgba(76,175,80,0.15)', border: '1px solid ' + (hibpResult.breached ? 'rgba(244,67,54,0.4)' : 'rgba(76,175,80,0.4)'), color: hibpResult.breached ? '#f44336' : '#4caf50' }}>
                  {hibpResult.breached ? 'Pwned ' + hibpResult.count.toLocaleString() + 'x' : 'Not Pwned'}
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.7rem' }}>
            <button onClick={() => { setResult(null); setInput(''); setHibpResult(null); }} style={{ padding: '0.8rem', background: '#07d2f8', border: 'none', borderRadius: '10px', color: '#000', fontWeight: '700', cursor: 'pointer', fontSize: '0.85rem' }}>New Scan</button>
            <button onClick={() => exportPDF(result)} style={{ padding: '0.8rem', background: 'transparent', border: '1px solid rgba(7,210,248,0.35)', borderRadius: '10px', color: '#07d2f8', fontWeight: '600', cursor: 'pointer', fontSize: '0.85rem' }}>Export PDF</button>
            <button onClick={() => sendEmailReport(result)} style={{ padding: '0.8rem', background: 'transparent', border: '1px solid rgba(7,210,248,0.35)', borderRadius: '10px', color: '#07d2f8', fontWeight: '600', cursor: 'pointer', fontSize: '0.85rem' }}>Email Report</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.8rem', flexWrap: 'wrap', justifyContent: 'center' }}>
        {[{ icon: '⚡', text: 'Instant' }, { icon: '🔒', text: 'Private' }, { icon: '🛡️', text: 'Free' }, { icon: '🔍', text: 'HIBP Check' }].map((b, i) => (
      ...
      <div style={{ width: '100%', maxWidth: '520px', background: 'rgba(7,210,248,0.03)', border: '1px solid rgba(7,210,248,0.2)', borderRadius: '16px', padding: '1.5rem', backdropFilter: 'blur(12px)', boxShadow: '0 4px 40px rgba(0,0,0,0.4)', position: 'relative', overflow: 'hidden', marginBottom: '1rem' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg,transparent,#07d2f8,transparent)' }} />
        {error && <div style={{ background: 'rgba(244,67,54,0.08)', border: '1px solid rgba(244,67,54,0.25)', borderRadius: '8px', padding: '0.6rem 0.85rem', color: '#f44336', fontSize: '0.8rem', marginBottom: '0.85rem' }}>⚠️ {error}</div>}
